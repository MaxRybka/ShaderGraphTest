...
Shader "Custom/HoloTriangles" {
	Properties
	{
		...
		_border("Border Scale", Range(0.01, 200)) = 1
		_posOffset("pos offset", Vector) = (1.0 , 1.0 , 2.0)
		_midPosOffset("mid Pos offset", Vector) = (1.0 , 1.0 , 2.0)
		...
		Pass
	{
		...
		float3 doMaterial(in float3 pos, in float3 midPos)
		{
			...
			float d = length(float2(pos.x*_posOffset.x, pos.z*_posOffset.z) - float2(midPos.x*_midPosOffset.x, midPos.z * _midPosOffset.z)) + pos.y*_posOffset.y - midPos.y*_midPosOffset.y;
			d = d / _RangeScale;
			...
			c += c2;
			// Cutoff front
			c *= smoothstep(_border, _border - 0.1, d);
			// Cutoff back
			c *= smoothstep(_border , _border - 0.1, d);
			// fade out
			c *= smoothstep(5.0, 4.0, _border);
